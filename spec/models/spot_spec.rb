...
RSpec.describe Spot, type: :model do
  ...
  describe '.move' do
    ...
    describe '0度方向(北)への移動' do
      let(:direction) { 0 }
      it 'lngは変化しないこと' do
        expect(Spot.__send__(:move, lat, lng, distance, direction)[:lng]).to eq lng
      end

      it 'latが増加すること' do
        expect(Spot.__send__(:move, lat, lng, distance, direction)[:lat]).to be > lat
      end
    end
    ...
    describe '90度方向(北)への移動' do
      let(:direction) { 90 }
      it 'latは変化しないこと' do
        expect(Spot.__send__(:move, lat, lng, distance, direction)[:lat]).to eq lat
      end

      it 'lngが増加すること' do
        expect(Spot.__send__(:move, lat, lng, distance, direction)[:lng]).to be > lng
      end
    end
